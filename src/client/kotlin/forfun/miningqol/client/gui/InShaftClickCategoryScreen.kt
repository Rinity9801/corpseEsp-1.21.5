...
class InShaftClickCategoryScreen(private val parentScreen: Screen) : VexelScreen("In Shaft Click Settings") {
    ...
    override fun afterInitialization() {
        ...
        Text("In Shaft Click Settings", 0xFFFFFFFF.toInt(), 28f, true)
            .setPositioning(0f, Pos.ParentCenter, 18f, Pos.ParentPixels)
            .childOf(mainPanel)
            .fadeIn(700, EasingType.EASE_OUT)

        Text("Auto clicker with cold-aware drill switching for mineshafts", 0xFF888888.toInt(), 13f, false)
        ...
        val sliderSpacing = 85f

        createSliderCard(
            "Mining Slot",
            1f, 9f,
            (InShaftClickManager.getMiningSlot() + 1).toFloat(),
            { value -> InShaftClickManager.setMiningSlot(value.toInt() - 1) },
            "",
            (mainPanel.width - toggleWidth) / 2f,
            sliderStartY,
            toggleWidth, 75f, mainPanel, 400L
        )

        createSliderCard(
            "Second Drill Slot",
            1f, 9f,
            (InShaftClickManager.getSecondDrillSlot() + 1).toFloat(),
            { value -> InShaftClickManager.setSecondDrillSlot(value.toInt() - 1) },
            "",
            (mainPanel.width - toggleWidth) / 2f,
            sliderStartY + sliderSpacing,
            toggleWidth, 75f, mainPanel, 500L
        )
        ...
        createSliderCard(
            "Cold Threshold",
            0f, 100f,
            InShaftClickManager.getColdThreshold().toFloat(),
    ...
    private fun createSliderCard(
        ...
        Rectangle(
            backgroundColor = 0xFF44AAFF.toInt(),
            borderRadius = 12f
        )
            .setSizing(5f, Size.Pixels, 100f, Size.ParentPerc)
            .setPositioning(0f, Pos.ParentPixels, 0f, Pos.ParentPixels)
        ...
        val valueText = Text("${initialValue.toInt()}$suffix", 0xFF44AAFF.toInt(), 14f, true)
        ...
        Slider(
            ...
            trackFillColor = 0xFF44AAFF.toInt(),
            thumbColor = 0xFF44AAFF.toInt(),
            trackHeight = 4f,
            thumbWidth = 16f,
            thumbHeight = 16f,
            thumbRadius = 8f,
