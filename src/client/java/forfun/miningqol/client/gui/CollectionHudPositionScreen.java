...
public class CollectionHudPositionScreen extends Screen {
    private final Screen parent;
    private boolean dragging = false;
    private int dragOffsetX = 0;
    private int dragOffsetY = 0;

    public CollectionHudPositionScreen(Screen parent) {
        super(Text.literal("Position Collection HUD"));
        this.parent = parent;
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawCenteredTextWithShadow(this.textRenderer,
            "§7Scale: " + String.format("%.1fx", CollectionTracker.getScale()) + " §8| §7Press ESC when done",
            this.width / 2, 35, 0xFFFFFF);

        if (dragging) {
            CollectionTracker.setPosition(mouseX - dragOffsetX, mouseY - dragOffsetY);
        }

        int x = CollectionTracker.getX();
        int y = CollectionTracker.getY();

        CollectionTracker.renderPreview(context);

        // Draw border around HUD
        int hudWidth = CollectionTracker.getWidth();
        int hudHeight = CollectionTracker.getHeight();
    ...
    public boolean mouseClicked(Click click, boolean firstClick) {
        if (click.button() == 0) {
            int hudX = CollectionTracker.getX();
            int hudY = CollectionTracker.getY();
            int hudWidth = CollectionTracker.getWidth();
            int hudHeight = CollectionTracker.getHeight();
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
        float newScale = CollectionTracker.getScale() + (float) (verticalAmount * 0.1);
        CollectionTracker.setScale(newScale);
        return true;
    }
